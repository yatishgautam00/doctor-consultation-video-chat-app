...
function MessageInput({ sendMessage, message, setMessage,image,setImage }) {
  ...
  const handleKeyDown = (e) => {
    if (e.key === 'Enter') {
      e.preventDefault(); // Prevent the default action to avoid line break
      sendMessage();
    }
  };

  return (
    <div className='relative flex items-center gap-2 p-4 border-t border-gray-200 bg-slate-200'>
    <FaPaperclip
    ...
    <input
      ...
      className='flex-1  p-2 bg-slate-200 border-b-2 border-r-[1px] border-t-[1px] border-t-slate-500 border-l-2 border-b-slate-500 border-r-slate-500 border-l-slate-500 rounded-xl outline-none'
