      ...
      ? doctorList.map((doctor, index) => {
          if (categorycond === "search") {
            ...
            return doctor.role === "doctor" &&
              doctor.category === category01 ? (
              <div
                key={index}
                className="border-[1px] rounded-lg p-3 hover:scale-105 bg-gray-25 bg-gray-50 cursor-pointer hover:border-primary hover:shadow-sm transition-all ease-in-out"
              >
                <Image
          ...
          if (!paramsCondition) {
            return doctor.role === "doctor" ? (
              <div
                key={index}
                className="border-[1px] shadow-md  hover:scale-105 rounded-lg p-2 md:p-3 bg-gray-25 bg-gray-50 cursor-pointer hover:border-primary hover:shadow-sm transition-all ease-in-out"
              >
                <Image
                  src={doctor.doctorImg}
                  alt="vaidyapadma doctor img"
